...
def render() -> None:
    ...
    with st.form("add_single_product", clear_on_submit=True):
        ap_code = st.text_input("Código", placeholder="Ej: 011")
        ap_name = st.text_input("Nombre", placeholder="Ej: Limpiavidrios")
    ...
    if add_submitted:
        ...
        if not code_s or not name_s:
            ...
            if inserted:
                ...
                st.success(f"Producto actualizado: {code_s}")
        ...
        with st.expander(f"{p.code} — {p.name}", expanded=False):
            ...
            if st.button("Guardar cambios", key=f"save:{p.code}"):
                update_product(
                    ...
                    code=p.code,
                    name=name.strip(),
                    category=category.strip() or "Sin categoría",
                    active=active,
                )
                st.success("Guardado.")
...
def _normalize_products_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["category"] = (
        ...
        .astype(str)
        .str.strip()
        .replace({"": "Sin categoría", "nan": "Sin categoría", "None": "Sin categoría"})
    )
