...
def render() -> None:
    ...
    st.caption("Registra el consumo o entrega diaria de implementos.")
    engine = get_engine()

    default_day = date.today() - timedelta(days=1)
    movement_date = st.date_input("Fecha de salida (por defecto: ayer)", value=default_day)
    ...
    with c1:
        code = st.text_input("Código", key="daily_out_code", placeholder="Ej: 001")
    with c2:
    ...
    save_clicked = csave.button(
        ...
        help="Si ya existe una carga para la fecha, usa las opciones de abajo.",
    )
    ...
    if save_clicked:
        ...
        try:
            ...
            insert_daily_out_batch(engine=engine, movement_date=movement_date, items=items, actor_role=ACTOR_ROLE)
    ...
    if closure_active:
        st.warning(
            ...
            "Puedes añadir más salidas o reemplazar la carga del día."
        )
        ...
        if a1.button("Añadir a este día (sumar salidas)"):
            try:
                ...
                append_daily_out_batch(engine=engine, movement_date=movement_date, items=items, actor_role=ACTOR_ROLE)
        ...
        if a2.button("Reemplazar carga del día"):
            try:
